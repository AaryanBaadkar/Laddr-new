...
router.get('/price-trends', async (req, res) => {
  try {
    ...
    const trends = Object.entries(locationMap).map(([location, props]) => {
      const stats = calculateStats(props);
      return {
        _id: location,
        avgPrice: stats ? Math.round(stats.avgPrice) : 0,
        avgPricePerSqFt: stats ? Math.round(stats.avgPricePerSqFt) : 0,
        count: stats ? stats.count : 0
      };
    }).filter(t => t.count > 0).sort((a, b) => b.avgPrice - a.avgPrice);
